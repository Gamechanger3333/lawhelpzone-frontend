...
import {
  Scale, Users, Briefcase, MessageSquare, Bell,
  Video, CheckCircle, AlertTriangle, X, RefreshCw,
...
const STATUS_MAP = {
  ...
  closed:        { label: "Closed",      color: "#10b981", bg: "#f0fdf4" },
};
...
export default function LawyerDashboard() {
  ...
  useEffect(() => {
    if (!tok()) { setLoading(false); setTimeout(() => setVisible(true), 50); return; }
    load();
    fetchBadges();
  ...
  const sendProposal = async () => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        setPropCase(null); setPropNote(""); setPropFee("");
        load(true);
      } else {
  ...
  const isAvailable   = lawyerProfile.isAvailable !== false;
  ...
  return (
    <>
      <style>{css}</style>
      <div className="ld-root" style={{ opacity: visible ? 1 : 0, transition: "opacity 0.4s" }}>
          ...
          <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.45)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(6px)", padding: 20 }}
            ...
            <div style={{ background: "#fff", border: "1px solid #f1f5f9", borderRadius: 20, maxWidth: 480, width: "100%", padding: 32, animation: "fadeUp 0.22s ease", boxShadow: "0 32px 64px rgba(0,0,0,0.15)", fontFamily: "'DM Sans', sans-serif" }}
              ...
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 22 }}>
                <div>
                  <h3 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: "#0f172a" }}>Submit Proposal</h3>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <span style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "3px 10px", borderRadius: 20, background: isAvailable ? "#f0fdf4" : "#fef2f2", color: isAvailable ? "#10b981" : "#ef4444", fontSize: 11, fontWeight: 700, border: `1px solid ${isAvailable ? "#bbf7d0" : "#fecaca"}` }}>
                  <span className={isAvailable ? "online-dot" : ""} style={isAvailable ? {} : { width: 8, height: 8, borderRadius: "50%", background: "#ef4444", display: "inline-block" }} />
            ...
            <div style={{ background: "#fff", border: "1px solid #f1f5f9", borderRadius: 16, overflow: "hidden", boxShadow: "0 2px 10px rgba(0,0,0,0.04)", animation: "fadeUp 0.4s ease 0.15s both" }}>
              ...
              {tab === "my" && (
                myCases.length === 0
                  ...
                  : myCases.map((c, i) => {
                      const sc     = STATUS_MAP[c.status] || STATUS_MAP.open;
                      ...
                      return (
                        <div key={c._id || i} className="case-row">
                          ...
                          <span style={{ padding: "3px 10px", borderRadius: 20, background: sc.bg, color: sc.color, fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{sc.label}</span>
                          {client._id && (
                            <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                              <button onClick={() => router.push(`/dashboard/lawyer/messages?contact=${client._id}`)} style={{ width: 30, height: 30, borderRadius: 8, background: "#eff6ff", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                                <MessageSquare size={13} style={{ color: "#3b82f6" }} />
                              </button>
                              <button onClick={() => router.push(`/dashboard/lawyer/video-calls?contact=${client._id}`)} style={{ width: 30, height: 30, borderRadius: 8, background: "#f0fdf4", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        ...
                        <div key={c._id || i} className="case-row" style={{ alignItems: "flex-start" }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                              ...
                              {c.urgency === "high" && (
                                <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 7px", borderRadius: 6, background: "#fef2f2", color: "#ef4444" }}>URGENT</span>
                              )}
                            </div>
                ...
                myClients.length === 0
                  ...
                  : myClients.map((cl, i) => (
                      <div key={cl._id || i} className="case-row">
                        ...
                        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                          <button onClick={() => router.push(`/dashboard/lawyer/messages?contact=${cl._id}`)} style={{ width: 30, height: 30, borderRadius: 8, background: "#eff6ff", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                            <MessageSquare size={13} style={{ color: "#3b82f6" }} />
                          </button>
                          <button onClick={() => router.push(`/dashboard/lawyer/video-calls?contact=${cl._id}`)} style={{ width: 30, height: 30, borderRadius: 8, background: "#f0fdf4", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                            <Video size={13} style={{ color: "#10b981" }} />
                          </button>
                        </div>
                      </div>
              ...
              <div style={{ background: "linear-gradient(160deg,#064e3b 0%,#065f46 100%)", padding: "20px 18px 18px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <p style={{ margin: 0, fontSize: 14.5, fontWeight: 700, color: "#f0fdf4" }}>{user?.name || "Lawyer"}</p>
          ...
          <div style={{ background: "#fff", border: "1px solid #f1f5f9", borderRadius: 16, overflow: "hidden", boxShadow: "0 2px 10px rgba(0,0,0,0.04)", animation: "fadeUp 0.4s ease 0.5s both" }}>
            <div style={{ padding: "18px 22px", borderBottom: "1px solid #f8fafc", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div>
                  <h3 style={{ margin: 0, fontSize: 14.5, fontWeight: 700, color: "#0f172a" }}>All Registered Users</h3>
              ...
              <div style={{ position: "relative" }}>
                ...
                <input value={userSearch} onChange={(e) => setUserSearch(e.target.value)} placeholder="Search members…"
                  className="ld-input" style={{ padding: "8px 12px 8px 30px", width: 210 }} />
              </div>
            </div>
            {filteredUsers.length === 0
              ? <div style={{ padding: "40px", textAlign: "center" }}><p style={{ color: "#94a3b8", margin: 0, fontSize: 13 }}>No users found</p></div>
                      ...
                      <div key={u._id} className="user-card">
                        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                          ...
                          <div style={{ minWidth: 0 }}>
                            ...
                            <span style={{ fontSize: 10, fontWeight: 700, padding: "1px 7px", borderRadius: 20, background: `${rc}14`, color: rc, textTransform: "capitalize" }}>{u.role}</span>
                          </div>
                        </div>
                        <div style={{ display: "flex", gap: 6 }}>
                          <button className="act-btn" onClick={() => router.push(`/dashboard/lawyer/messages?contact=${u._id}`)} style={{ background: "#eff6ff", color: "#3b82f6" }}>
                            <MessageSquare size={12} /> Message
                          </button>
                          <button className="act-btn" onClick={() => router.push(`/dashboard/lawyer/video-calls?contact=${u._id}`)} style={{ background: "#f0fdf4", color: "#10b981" }}>
                            <Video size={12} /> Call
                          </button>
                        </div>
                      </div>
