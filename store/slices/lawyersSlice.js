...
export const fetchLawyers = createAsyncThunk(
  ...
  async ({ specialization, search, page = 1 } = {}, { rejectWithValue }) => {
    try {
      ...
      const res  = await fetch(`${API}/api/lawyers?${params}`, { credentials: "include", headers: H() });
...
export const fetchLawyerById = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      const res  = await fetch(`${API}/api/lawyers/${id}`, { credentials: "include", headers: H() });
      const data = await res.json();
      if (!res.ok) return rejectWithValue(data.message);
      return data;
    } catch (err) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchLawyers.fulfilled, (state, action) => {
        ...
        state.pagination = {
          page: action.payload.page, pages: action.payload.pages, total: action.payload.total,
        };
      })
      .addCase(fetchLawyers.rejected,  (state, action) => { state.loading = false; state.error = action.payload; })

      .addCase(fetchLawyerById.pending,   (state) => { state.loading = true; state.selectedLawyer = null; })
      .addCase(fetchLawyerById.fulfilled, (state, action) => { state.loading = false; state.selectedLawyer = action.payload; })
      .addCase(fetchLawyerById.rejected,  (state, action) => { state.loading = false; state.error = action.payload; })

      .addCase(verifyLawyer.fulfilled,  (state, action) => {
        state.lawyers = state.lawyers.filter((l) => l._id !== action.payload.id);
      })
      .addCase(verifyLawyer.rejected,   (state, action) => { state.error = action.payload; })

      .addCase(updateLawyerProfile.fulfilled, (state, action) => { state.selectedLawyer = action.payload; })
