...
const css = `
  ...
  .section-row:hover { background: rgba(10,26,63,0.03) !important; }
...
function CaseClassifier() {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      <div>
        <label style={{ display:"block", fontSize:11, fontWeight:700, color:"#64748b", marginBottom:6, letterSpacing:"0.06em", textTransform:"uppercase" }}>Case Title</label>
        <input style={inp} placeholder="e.g. Wrongful termination from employer"
          value={form.title} onChange={e => setForm(f => ({ ...f, title:e.target.value }))} />
      </div>
      <div>
        <label style={{ display:"block", fontSize:11, fontWeight:700, color:"#64748b", marginBottom:6, letterSpacing:"0.06em", textTransform:"uppercase" }}>Describe Your Legal Issue</label>
        <textarea style={{ ...inp, minHeight:110, resize:"vertical", lineHeight:1.6 }}
          placeholder="Describe your situation in detail. The more context you provide, the more accurate the AI classification..."
          value={form.description} onChange={e => setForm(f => ({ ...f, description:e.target.value }))} />
        <p style={{ margin:"4px 0 0", fontSize:11, color:"#94a3b8" }}>{form.description.length} characters</p>
      </div>
      ...
      {result && (
        <div style={{ background:"#f8fafc", borderRadius:16, padding:20, border:"1px solid #e2e8f0", animation:"fd 0.4s ease", display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
            {[
              ...
              <div key={label} style={{ background:"#fff", borderRadius:10, padding:"12px 14px", border:"1px solid #f1f5f9" }}>
                <p style={{ margin:"0 0 3px", fontSize:10, color:"#94a3b8", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.06em" }}>{label}</p>
                <p style={{ margin:0, fontSize:13, fontWeight:700, color:NAVY }}>{value}</p>
              </div>
            ))}
            <div style={{ background:"#fff", borderRadius:10, padding:"12px 14px", border:"1px solid #f1f5f9" }}>
              <p style={{ margin:"0 0 6px", fontSize:10, color:"#94a3b8", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.06em" }}>Urgency</p>
            ...
            <div style={{ background:"#fff", borderRadius:10, padding:"12px 14px", border:"1px solid #f1f5f9" }}>
              <p style={{ margin:"0 0 3px", fontSize:10, color:"#94a3b8", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.06em" }}>Complexity</p>
          ...
          <div style={{ background:"#fff", borderRadius:10, padding:"12px 14px", border:"1px solid #f1f5f9" }}>
            <p style={{ margin:"0 0 5px", fontSize:10, color:"#94a3b8", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.06em" }}>Case Summary</p>
            <p style={{ margin:0, fontSize:13, color:"#374151", lineHeight:1.7 }}>{result.summary}</p>
          </div>

          {result.keyIssues?.length > 0 && (
            <div style={{ background:"#fff", borderRadius:10, padding:"12px 14px", border:"1px solid #f1f5f9" }}>
              <p style={{ margin:"0 0 8px", fontSize:10, color:"#94a3b8", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.06em" }}>Key Issues</p>
          ...
          <p style={{ margin:0, fontSize:11, color:"#94a3b8", fontStyle:"italic" }}>⚠️ AI classification is a suggestion only — final categorization may differ after lawyer review.</p>
        </div>
      )}
    </div>
  );
...
function CopyBtn({ text }) {
  ...
  return (
    <button className="copy-btn" onClick={() => { navigator.clipboard.writeText(text); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
      style={{ display:"flex", alignItems:"center", gap:4, padding:"3px 9px", borderRadius:7, border:"1px solid #e2e8f0", background:"#fff", fontSize:11, fontWeight:600, color:"#64748b", cursor:"pointer", transition:"all 0.15s" }}>
...
function DocumentAnalyzer() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      else setError(data.message || "Analysis failed. Try again.");
    } catch { setError("Connection error. Please try again."); }
    finally { setLoad(false); }
  };

  const sections = [
    { key:"keyPoints",      label:"📋 Key Points",       items:result?.keyPoints },
    { key:"parties",        label:"👥 Parties Involved",  items:result?.parties },
    { key:"importantDates", label:"📅 Important Dates",   items:result?.importantDates },
    { key:"warnings",       label:"⚠️ Warnings",          items:result?.warnings },
  ];

  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      <div>
        <label style={{ display:"block", fontSize:11, fontWeight:700, color:"#64748b", marginBottom:6, letterSpacing:"0.06em", textTransform:"uppercase" }}>Document Name (optional)</label>
        <input style={inp} placeholder="e.g. Employment Contract, Rental Agreement"
          value={fileName} onChange={e => setName(e.target.value)} />
      </div>
      <div>
        <label style={{ display:"block", fontSize:11, fontWeight:700, color:"#64748b", marginBottom:6, letterSpacing:"0.06em", textTransform:"uppercase" }}>Paste Document Text</label>
        <textarea style={{ ...inp, minHeight:130, resize:"vertical", lineHeight:1.6 }}
        ...
        <p style={{ margin:"4px 0 0", fontSize:11, color:"#94a3b8" }}>{text.length} characters (max 3,000 analyzed)</p>
      </div>
      ...
      {result && (
        <div style={{ display:"flex", flexDirection:"column", gap:10, animation:"fd 0.4s ease" }}>
          ...
          <div style={{ background:"#f8fafc", borderRadius:12, padding:"14px 16px", border:"1px solid #e2e8f0" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8 }}>
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <p style={{ margin:0, fontSize:10, color:"#94a3b8", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.06em" }}>Document Summary</p>
          ...
          {sections.map(({ key, label, items }) =>
            items?.length > 0 ? (
              <div key={key} style={{ background:"#f8fafc", borderRadius:12, border:"1px solid #e2e8f0", overflow:"hidden" }}>
                <button className="section-row tab-btn" onClick={() => toggle(key)}
                  style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"11px 16px", background:"transparent", fontSize:13, fontWeight:700, color:"#0f172a", textAlign:"left" }}>
                  {label}
                  {open[key] ? <ChevronUp size={13} style={{ color:"#94a3b8" }} /> : <ChevronDown size={13} style={{ color:"#94a3b8" }} />}
                </button>
                {open[key] && (
                  <div style={{ padding:"0 16px 12px", borderTop:"1px solid #e2e8f0", paddingTop:10, display:"flex", flexDirection:"column", gap:6 }}>
                    {items.map((item, i) => (
                      <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:8, fontSize:13, color:"#374151", lineHeight:1.6 }}>
          ...
          <p style={{ margin:0, fontSize:11, color:"#94a3b8", fontStyle:"italic" }}>⚠️ AI analysis is for informational purposes only. Always have a qualified lawyer review legal documents.</p>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ maxWidth:740, margin:"0 auto", padding:"28px 24px", animation:"fd 0.5s ease" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20 }}>
        ...
        <div>
          <h1 style={{ margin:0, fontSize:21, fontWeight:800, color:"#0f172a" }}>AI Legal Tools</h1>
          <p style={{ margin:0, fontSize:12, color:"#64748b" }}>Powered by Google Gemini</p>
        </div>
      </div>
      ...
      <div style={{ background:"#fff", borderRadius:18, border:"1px solid #e2e8f0", padding:"22px 24px", boxShadow:"0 2px 12px rgba(0,0,0,0.05)" }}>
