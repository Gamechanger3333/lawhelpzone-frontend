      ...
      {showAllEmoji ? (
        ...
        <>
          ...
          <div style={{ display: "flex", alignItems: "center", padding: "10px 8px 8px", borderBottom: `1px solid ${BORDER}`, gap: 2 }}>
            ...
            <button onClick={(e) => { e.stopPropagation(); setShowAllEmoji(true); }}
    ...
    try {
      ...
      mr.onstop = async () => {
        ...
        try {
          ...
          await sendMsg({ ...att, type: "audio/" + ext });
        } catch { alert("Voice upload failed — check backend /api/upload accepts audio."); }
        ...
        {(isMobile ? !showSidebar : true) && (
          activeId ? (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "var(--chat-bg)", minWidth: 0, backgroundImage: "radial-gradient(circle at 1px 1px, rgba(0,0,0,0.03) 1px, transparent 0)", backgroundSize: "20px 20px" }}>
                ...
                {loadM ? (
                  ...
                  <div key={day}>
                    ...
                    {dayMsgs.map((msg, i) => {
                      ...
                      const isAudio   = fileUrl && (msg.type === "audio" || /\.(webm|ogg|mp3|wav|m4a)$/i.test(msg.fileName || ""));
                      const msgType   = fileUrl ? (msg.type || "text") : "text";
                      const isImg     = msgType === "image" && !!fileUrl && !isAudio;
                      // Guard: never render the literal string "Attachment" as message text
                      const msgContent = msg.content && msg.content !== "Attachment" ? msg.content : "";
                            ...
                            <div className="msg-b" style={{ padding: isImg ? 4 : "9px 13px", borderRadius: msg.replyTo ? (mine ? "0 14px 4px 14px" : "14px 0 14px 4px") : (mine ? "18px 18px 4px 18px" : "4px 18px 18px 18px"), background: mine ? "#3b82f6" : "var(--bubble-other-bg)", color: mine ? "#fff" : "var(--bubble-other-color)", border: mine ? "none" : "1px solid var(--border-color)", boxShadow: "0 2px 8px rgba(0,0,0,0.08)", opacity: msg.pending ? 0.65 : 1 }}>
                              ...
                              {msgContent && (
                                <p style={{ margin: 0, fontSize: 14, lineHeight: 1.55, wordBreak: "break-word" }}>
                                  {msgSearch ? msgContent.split(new RegExp(`(${msgSearch})`, "gi")).map((part, idx) =>
                              ...
                              {fileUrl && isAudio && (
                                <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 200 }}>
                                  <audio controls src={fileUrl} style={{ height: 32, flex: 1, outline: "none", filter: mine ? "invert(1) hue-rotate(180deg)" : "none" }} preload="metadata" />
                                </div>
                              )}
              ...
              {recording && (
                <div style={{ padding: "10px 14px", background: "#fff1f2", borderTop: "1px solid #fecdd3", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
                  <div style={{ width: 10, height: 10, borderRadius: "50%", background: "#ef4444", flexShrink: 0, animation: "recPulse 1s ease infinite" }} />
                  ...
                  <button onClick={cancelRecording} style={{ padding: "6px 12px", borderRadius: 8, border: "none", background: "#fee2e2", color: "#ef4444", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                    Cancel
                  </button>
                  <button onClick={stopRecording} style={{ padding: "6px 14px", borderRadius: 8, border: "none", background: "#ef4444", color: "#fff", fontWeight: 700, fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}>
                    <Send size={12} /> Send
                  </button>
                </div>
              )}
              ...
              <div style={{ padding: isMobile ? "8px 10px" : "10px 14px", background: "var(--header-bg)", borderTop: "1px solid var(--border-color)", display: "flex", gap: 6, alignItems: "flex-end", flexShrink: 0, position: "relative" }}>
                ...
                {/* Mic button — only shown when input is empty and not editing */}
