"use client";
// v2026-03-15-MERGED
// ✅ OLD UI: clean simple design from original page.jsx
// ✅ NEW: Voice notes (record, play, WhatsApp waveform)
// ✅ NEW: Ghost message filtering (no Attachment/Voice message labels)
// ✅ NEW: Reactions + reply + edit + delete
// ✅ NEW: Typing indicator
// ✅ NEW: Online status
// ✅ NEW: Read receipts ✓/✓✓
// ✅ NEW: Emoji picker (full 500+)
// ✅ NEW: Message search
// ✅ NEW: Profile modal
...
import {
  ...
  Check, CheckCheck, Download, Star, User, Mic, AlertCircle,
} from "lucide-react";
...
// ── Simple emoji list (old style) + full set for picker ──────────────────────
const EMOJIS_QUICK = ["😊","😂","👍","❤️","🙏","✅","🔥","💯","⚖️","📋","💼","📞","✉️","🎉","🤝","📌","👋","🌟","💪","🎯"];
const EMOJI_CATS = {
  "😊":["😀","😃","😄","😁","😆","😅","🤣","😂","🙂","🙃","😉","😊","😇","🥰","😍","🤩","😘","😚","😋","😛","😜","🤪","😝","🤑","🤗","🤭","🤫","🤔","🤐","🤨","😐","😑","😶","😏","😒","🙄","😬","🤥","😌","😔","😪","🤤","😴","😷","🤒","🤕","🤢","🤮","🤧","🥵","🥶","🥴","😵","💫","🤯","🤠","🥳","😎","🤓","🧐","😕","😟","🙁","☹️","😮","😯","😲","😳","🥺","😦","😧","😨","😰","😥","😢","😭","😱","😖","😣","😞","😓","😩","😫","🥱","😤","😡","😠","🤬","😈","👿","💀","☠️","💩","🤡","👹","👺","👻","👽","👾","🤖"],
  "👋":["👋","🤚","🖐️","✋","🖖","👌","🤌","🤏","✌️","🤞","🤟","🤘","🤙","👈","👉","👆","👇","☝️","👍","👎","✊","👊","🤛","🤜","👏","🙌","👐","🤲","🤝","🙏","✍️","💅","🤳","💪","🦾","🦵","🦶","👂","🦻","👃","🧠","🦷","🦴","👀","👁️","👅","👄","💋"],
  "❤️":["❤️","🧡","💛","💚","💙","💜","🖤","🤍","🤎","💔","❤️‍🔥","❤️‍🩹","❣️","💕","💞","💓","💗","💖","💘","💝","💟"],
...
const fmt = (d) => { try { return new Date(d).toLocaleTimeString([], { hour:"2-digit", minute:"2-digit" }); } catch { return ""; } };
const fmtDay = (d) => {
  try {
...
const fmtSec = (s) => { const n = (!s||!isFinite(s)||isNaN(s))?0:Math.max(0,s); return `${Math.floor(n/60)}:${String(Math.floor(n%60)).padStart(2,"0")}`; };
...
function AudioBubble({ src, mine }) {
  ...
  useEffect(() => {
    ...
    const onTime = () => { if (a.duration&&isFinite(a.duration)&&a.duration>0) { setProgress(a.currentTime/a.duration*100); setElapsed(a.currentTime); } };
  ...
  const BARS = 28;
  const barH = Array.from({length:BARS},(_,i)=>20+Math.abs(Math.sin(i*1.9)*55+Math.cos(i*0.8)*28));
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:10, minWidth:200, maxWidth:260, padding:"2px 0" }}>
      ...
      <button onClick={toggle} style={{ width:40, height:40, borderRadius:"50%", border:"none", background:loadErr?(mine?"rgba(255,255,255,0.15)":"#fee2e2"):mine?"rgba(255,255,255,0.25)":"#e8f5e9", cursor:loadErr?"not-allowed":"pointer", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, transition:"transform 0.1s" }}
        onMouseEnter={e=>{if(!loadErr)e.currentTarget.style.transform="scale(1.1)"}}
        onMouseLeave={e=>{e.currentTarget.style.transform="scale(1)"}}>
        {loadErr ? <span style={{fontSize:13}}>⚠️</span>
          : playing ? <span style={{display:"flex",gap:2.5,alignItems:"flex-end",height:14}}><span style={{width:3,height:14,background:mine?"#fff":"#25d366",borderRadius:2}} /><span style={{width:3,height:14,background:mine?"#fff":"#25d366",borderRadius:2}} /></span>
          : <span style={{fontSize:15,marginLeft:2,color:mine?"#fff":"#25d366"}}>▶</span>}
      </button>
      <div style={{ flex:1, display:"flex", flexDirection:"column", gap:3 }}>
        <div style={{ display:"flex", alignItems:"center", gap:1.5, height:26, cursor:"pointer" }}
          onClick={e=>{ const a=audioRef.current; if(!a||!a.duration||!isFinite(a.duration))return; const r=e.currentTarget.getBoundingClientRect(); a.currentTime=(e.clientX-r.left)/r.width*a.duration; }}>
          {barH.map((h,i)=>(
            <div key={i} style={{ flex:1, borderRadius:2, minWidth:2, height:`${h}%`, maxHeight:24, background:(i/BARS*100)<=progress?fill:empty, transition:"background 0.1s" }} />
          ))}
        </div>
        <span style={{ fontSize:10, color:mine?"rgba(255,255,255,0.7)":"#666", fontWeight:600, userSelect:"none" }}>
...
function ProfileModal({ user:u, onClose, onMessage, onCall }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", zIndex:2000, display:"flex", alignItems:"center", justifyContent:"center", padding:16, backdropFilter:"blur(4px)" }}
...
function MsgMenu({ msg, isMine, onDelete, onEdit, onReply, onReact, onCopy, onStar, onClose, x, y }) {
  ...
  const [showAllEmoji, setShowAll] = useState(false);
  ...
  const menuW = showAllEmoji?300:220, menuH = showAllEmoji?400:320;
  ...
  return (
    <div ref={ref} style={{ position:"fixed", left:safeX, top:safeY, zIndex:9999, background:BG, borderRadius:16, boxShadow:"0 16px 56px rgba(0,0,0,0.2), 0 2px 8px rgba(0,0,0,0.07)", border:`1px solid ${BORDER}`, width:menuW, height:showAllEmoji?menuH:"auto", overflow:"hidden", animation:"popIn 0.15s ease", display:"flex", flexDirection:"column", transition:"width 0.18s,height 0.18s" }}>
      {showAllEmoji ? (
        <div style={{display:"flex",flexDirection:"column",flex:1,minHeight:0}}>
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px 6px",borderBottom:`1px solid ${BORDER}`}}>
            <button onClick={()=>setShowAll(false)} style={{width:26,height:26,borderRadius:7,border:"none",background:HOVER,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,color:TXT,fontWeight:700}}>‹</button>
            <div style={{display:"flex",overflowX:"auto",gap:1,flex:1,scrollbarWidth:"none"}}>
              {Object.keys(EMOJI_CATS).map(cat=>(
                <button key={cat} onClick={()=>setEmojiCat(cat)} title={cat}
                  style={{flexShrink:0,width:28,height:26,borderRadius:6,border:"none",background:emojiCat===cat?HOVER:"transparent",fontSize:16,cursor:"pointer",outline:emojiCat===cat?`2px solid #3b82f6`:"none"}}>
                  {cat}
                </button>
              ))}
            </div>
          </div>
          <div style={{flex:1,overflowY:"auto",padding:"6px",display:"grid",gridTemplateColumns:"repeat(8,1fr)",gap:2}}>
            {(EMOJI_CATS[emojiCat]||[]).map((e,i)=>(
              <button key={`${e}-${i}`} onClick={()=>{onReact(e);onClose();}}
                style={{width:"100%",aspectRatio:"1",border:"none",background:"transparent",fontSize:19,cursor:"pointer",borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",transition:"background 0.1s,transform 0.1s"}}
        ...
        <>
          ...
          <div style={{display:"flex",alignItems:"center",padding:"10px 8px 8px",borderBottom:`1px solid ${BORDER}`,gap:2}}>
            <div style={{display:"flex",overflowX:"auto",flex:1,gap:1,scrollbarWidth:"none"}}>
              {QUICK_REACTIONS.map(r=>(
                <button key={r} onClick={()=>{onReact(r);onClose();}}
                  style={{flexShrink:0,width:32,height:32,borderRadius:9,border:"none",background:"transparent",fontSize:19,cursor:"pointer",transition:"transform 0.12s,background 0.12s"}}
            ...
            <button onClick={e=>{e.stopPropagation();setShowAll(true);}}
              style={{flexShrink:0,width:32,height:32,borderRadius:9,border:`1.5px solid ${BORDER}`,background:HOVER,fontSize:17,fontWeight:800,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:MUTED,transition:"background 0.12s"}}
              onMouseEnter={ev=>ev.currentTarget.style.background="#e2e8f0"}
              onMouseLeave={ev=>ev.currentTarget.style.background=HOVER}>
              ＋
            </button>
          </div>
...
function NewConvPanel({ onClose, onSelect, myId, isMobile }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", zIndex:1500, backdropFilter:"blur(3px)" }}
      onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{ width:isMobile?"100%":300, height:"100vh", background:"var(--sidebar-bg,#fff)", display:"flex", flexDirection:"column", boxShadow:"4px 0 20px rgba(0,0,0,0.14)", animation:"slideIn 0.25s ease" }}>
        <div style={{ padding:"16px 14px 12px", borderBottom:"1px solid var(--border-color,#e2e8f0)" }}>
          ...
          <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:8}}>
            {[["all","All"],["client","Clients"],["lawyer","Lawyers"],["admin","Admins"]].map(([k,l])=>(
              <button key={k} onClick={()=>setRoleFilter(k)}
                style={{padding:"3px 9px",borderRadius:20,border:`1.5px solid ${roleFilter===k?"#3b82f6":"var(--border-color,#e2e8f0)"}`,background:roleFilter===k?"#eff6ff":"transparent",color:roleFilter===k?"#3b82f6":"var(--text-muted,#94a3b8)",fontSize:11,fontWeight:700,cursor:"pointer"}}>
                {l} ({counts[k]})
              </button>
            ))}
          </div>
...
function MessagesContent() {
  ...
  const fetchUserInfo=useCallback(async(id)=>{
    for(const url of[`${API}/api/users/${id}`,`${API}/api/lawyers/${id}`,`${API}/api/admin/users/${id}`]){
      try{const r=await fetch(url,{credentials:"include",headers:HJ()});if(r.ok){const d=await r.json();const u=d.user||d.lawyer||d;if(u?._id)return u;}}catch{}
    }
  ...
  const loadContacts=useCallback(async(silent=false)=>{
    ...
    try{
      ...
      if(r.ok){const d=await r.json();const list=d.contacts||[];setContacts(list);const online={};list.forEach(c=>{if(c.isOnline)online[c._id]=true;});setOnline(p=>({...p,...online}));}
    }catch{}
  ...
  const loadMsgs=useCallback(async(contactId,silent=false)=>{
    ...
    try{
      ...
      if(r.ok){const d=await r.json();setMsgs(d.messages||[]);if(!silent)fetch(`${API}/api/messages/${contactId}/read`,{method:"PATCH",credentials:"include",headers:HJ()}).catch(()=>{});}
    }catch{}
  ...
  const deleteMsg=useCallback(async(msgId,mode)=>{
    ...
    if(mode==="everyone"){try{await fetch(`${API}/api/messages/${msgId}`,{method:"DELETE",credentials:"include",headers:HJ()});}catch{setDeleted(p=>{const n=new Set(p);n.delete(msgId);return n;});}}
  },[]);

  const submitEdit=useCallback(async()=>{
    if(!editingMsg||!text.trim())return;
    if(!canEdit(editingMsg)){alert("Edit window expired (15 min).");return;}
    ...
    try{await fetch(`${API}/api/messages/${editingMsg._id}`,{method:"PATCH",credentials:"include",headers:HJ(),body:JSON.stringify({content:newText})});}catch{loadMsgs(activeId,true);}
  ...
  const sendMsg=useCallback(async(attachment=null)=>{
    ...
    try{
      ...
      if(attachment){body.fileUrl=attachment.url;body.fileName=attachment.name;body.type=attachment.msgType||((attachment.type||"").startsWith("audio/")?"audio":(attachment.type||"").startsWith("image/")?"image":"file");}
      const r=await fetch(`${API}/api/messages`,{method:"POST",credentials:"include",headers:HJ(),body:JSON.stringify(body)});
      if(r.ok){const d=await r.json();setMsgs(p=>p.map(m=>m._id===opt._id?(d.message||d):m));if(!activeId&&contactParam)setActiveId(contactParam);loadContacts(true);}
  ...
  const selectContact=(c)=>{
    ...
    setMsgSearch("");setShowMsgSrch(false);setReplyTo(null);setEditing(null);setText("");
    setTimeout(()=>autoResize(),0);
  ...
  const startRecording=async()=>{
    try{
      ...
      const mimeType=["audio/webm;codecs=opus","audio/webm","audio/ogg;codecs=opus","audio/ogg","audio/mp4",""].find(t=>{try{return!t||MediaRecorder.isTypeSupported(t);}catch{return false;}})||"";
  ...
  // Effects
  useEffect(()=>{if(!user){setTimeout(()=>setReady(true),500);return;}loadContacts();const p=setInterval(()=>loadContacts(true),8000);return()=>clearInterval(p);},[user]);
  ...
  useEffect(()=>{
    ...
    const tp=setInterval(async()=>{try{const r=await fetch(`${API}/api/messages/${activeId}/typing-status`,{credentials:"include",headers:HJ()});if(r.ok){const d=await r.json();setIsTyping(!!d.isTyping);}}catch{}},2500);
    return()=>{clearInterval(t);clearInterval(tp);};
  },[activeId]);
  useEffect(()=>{bottomRef.current?.scrollIntoView({behavior:"smooth"});},[messages]);
  useEffect(()=>{if(!loadC)setTimeout(()=>setReady(true),80);},[loadC]);

  // Ghost message detection
  const _voiceRe=/voice\s*(message|note|recording)?/i;
  const _audioExtRe=/\.(webm|ogg|mp3|wav|m4a|aac|flac|opus)(\?|$)/i;
  const JUNK=new Set(["attachment","📎 attachment","📎 file","file","📁 file","voice message","Voice message","voice note","Voice note","voice recording","voice","audio","🔊","🎙️","🎤","sent a file","sent an attachment","sent a voice note","sent a voice message"]);
  const isPlaceholder=(s)=>{if(!s)return true;const t=s.trim();if(!t)return true;if(JUNK.has(t)||JUNK.has(t.toLowerCase()))return true;if(/^(voice\s*(message|note|recording)?|🎤|🔊|📎\s*(file|attachment)?|attachment)$/i.test(t))return true;return false;};
  const cleanContent=(c)=>isPlaceholder(c)?"":c;
  ...
  return (
    <>
      <style>{`
        :root{--sk:#e2e8f0;--chat-bg:#f8fafc;--sidebar-bg:#ffffff;--header-bg:#ffffff;--input-bg:#f8fafc;--input-border:#e2e8f0;--conv-hover:#f1f5f9;--border-color:#e2e8f0;--bubble-other-bg:#ffffff;--bubble-other-color:#0f172a;--text-heading:#0f172a;--text-muted:#64748b;--text-primary:#374151;}
      ...
      <div style={{ height:isMobile?"calc(100vh - 80px)":"calc(100vh - 112px)", display:"flex", borderRadius:isMobile?0:20, overflow:"hidden", border:isMobile?"none":"1px solid var(--border-color)", boxShadow:isMobile?"none":"0 4px 24px rgba(0,0,0,0.07)", opacity:ready?1:0, transition:"opacity 0.4s" }}>
        ...
        activeId?(
          <div style={{flex:1,display:"flex",flexDirection:"column",background:"var(--chat-bg)",minWidth:0}}>
            ...
            {/* Header */}
            <div style={{padding:isMobile?"10px 12px":"12px 20px",background:"var(--header-bg)",borderBottom:"1px solid var(--border-color)",display:"flex",alignItems:"center",gap:12,flexShrink:0,boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
            ...
            {/* Search bar - sticky */}
            {showMsgSrch&&(
              <div style={{padding:"8px 14px",background:"var(--header-bg)",borderBottom:"1px solid var(--border-color)",display:"flex",gap:8,alignItems:"center",flexShrink:0,position:"sticky",top:0,zIndex:9}}>
                ...
                {msgSearch&&<span style={{fontSize:11,color:"var(--text-muted)",whiteSpace:"nowrap"}}>{displayMsgs.length} result{displayMsgs.length!==1?"s":""}</span>}
              ...
              {loadM?(
                ...
                <div key={day}>
                  ...
                  {dayMsgs.map((msg,i)=>{
                    ...
                    // Detect audio
                    const isAudio=(
                      msg.type==="audio"||(msg.type||"").startsWith("audio/")||
                      _audioExtRe.test(msg.fileUrl||"")||_audioExtRe.test(msg.fileName||"")||
                      (msg.fileUrl||"").startsWith("data:audio")||
                      _voiceRe.test(msg.fileName||"")||_voiceRe.test(msg.content||"")
                    );

                    // Skip ghost messages
                    if(isAudio&&!fileUrl&&!msg.fileUrl)return null;
                    const contentIsJunk=isPlaceholder(msg.content);
                    const fileNameIsJunk=!msg.fileName||isPlaceholder(msg.fileName);
                    if(contentIsJunk&&fileNameIsJunk&&!msg.fileUrl?.startsWith("data:"))return null;
                    if(!isAudio&&!fileUrl&&!cleanContent(msg.content))return null;
                    ...
                    return(
                      <div key={msg._id||i} className="msg-wrap"
                        ...
                        <div style={{maxWidth:isMobile?"83%":"68%",display:"flex",flexDirection:"column"}}>
                          {/* Reply quote */}
                          {msg.replyTo&&(
                          ...
                          <div className="msg-b" style={{
                            ...
                            background:mine?(isAudio?"#005c4b":"#3b82f6"):(isAudio?"#f0fdf4":"var(--bubble-other-bg)"),
                            color:mine?"#fff":"var(--bubble-other-color)",
                            border:mine?"none":(isAudio?"1px solid #86efac":"1px solid var(--border-color)"),
                            boxShadow:"0 2px 8px rgba(0,0,0,0.06)",
                            opacity:msg.pending?0.6:1,
                          }}>
                            {isStarred&&<span style={{fontSize:10,marginBottom:3,display:"block",opacity:0.7}}>⭐</span>}
                            {content&&(
                            ...
                            {!isAudio&&isImg&&fileUrl&&(
                              <a href={fileUrl} target="_blank" rel="noreferrer">
                                <img src={fileUrl} alt={msg.fileName||"image"} style={{maxWidth:isMobile?180:220,maxHeight:200,borderRadius:12,display:"block",objectFit:"cover",marginTop:content?6:0}}/>
                              </a>
                            )}
                            {!isAudio&&!isImg&&fileUrl&&(
                              <a href={fileUrl} target="_blank" rel="noreferrer"
                                ...
                                <div style={{minWidth:0}}>
                                  <p style={{margin:0,fontSize:12,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:150}}>{(!msg.fileName||JUNK.has(msg.fileName.trim().toLowerCase()))?"📎 Download file":msg.fileName}</p>
                            ...
                            <div style={{display:"flex",gap:3,flexWrap:"wrap",marginTop:4,justifyContent:mine?"flex-end":"flex-start"}}>
                              {Object.entries(reactions).map(([emoji,users])=>(
                                <button key={emoji} onClick={()=>addReaction(msg._id,emoji)}
                                  style={{padding:"2px 7px",borderRadius:12,background:users.includes(myId)?"#dbeafe":"rgba(255,255,255,0.9)",border:`1px solid ${users.includes(myId)?"#93c5fd":"rgba(0,0,0,0.1)"}`,cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",gap:3}}>
            ...
            {(replyTo||editingMsg)&&(
              <div style={{padding:"8px 14px",background:editingMsg?"#fffbeb":"#eff6ff",borderTop:`1px solid ${editingMsg?"#fde68a":"#bfdbfe"}`,display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
                <div style={{flex:1,minWidth:0}}>
                  <p style={{margin:0,fontSize:11,fontWeight:700,color:editingMsg?"#d97706":"#3b82f6"}}>{editingMsg?"✏️ Editing (15 min limit)":`↩ Replying to ${activeInfo?.name||"message"}`}</p>
            ...
            {recording&&(
              <div style={{padding:"10px 14px",background:"var(--header-bg)",borderTop:"1px solid var(--border-color)",display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
                <button onClick={cancelRecording} style={{width:38,height:38,borderRadius:"50%",border:"none",background:"#fee2e2",color:"#ef4444",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  <Trash2 size={16}/>
                </button>
                <div style={{display:"flex",alignItems:"center",gap:8,flex:1,background:"var(--input-bg)",borderRadius:24,padding:"6px 14px",border:"2px solid #ef4444"}}>
                  ...
                  <span style={{fontSize:14,fontWeight:800,color:"#ef4444",minWidth:38,fontFamily:"monospace"}}>{fmtSec(recSeconds)}</span>
                  <div style={{flex:1,display:"flex",alignItems:"center",gap:1.5,height:26,overflow:"hidden"}}>
                    {Array.from({length:26}).map((_,idx)=>(
                      <div key={idx} style={{flex:1,borderRadius:2,background:"#ef4444",opacity:0.5+(idx%3)*0.15,height:`${15+Math.abs(Math.sin(idx*0.9+recSeconds*0.8)*65)}%`,transition:"height 0.15s ease"}}/>
                    ))}
                  </div>
                  <Mic size={13} style={{color:"#ef4444",flexShrink:0}}/>
                </div>
                <button onClick={stopRecording} style={{width:38,height:38,borderRadius:"50%",border:"none",background:"#25d366",color:"#fff",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  <Send size={16}/>
                </button>
              </div>
            )}
            ...
            {/* Input bar */}
            {!recording&&(
            <div style={{padding:isMobile?"8px 10px":"10px 14px",background:"var(--header-bg)",borderTop:"1px solid var(--border-color)",display:"flex",gap:8,alignItems:"flex-end",flexShrink:0,position:"relative"}}>
              {/* Emoji picker */}
              {showEmoji&&(
                <div style={{position:"fixed",bottom:70,left:isMobile?4:60,zIndex:9000,background:"var(--sidebar-bg,#fff)",borderRadius:14,boxShadow:"0 -4px 32px rgba(0,0,0,0.15)",border:"1px solid var(--border-color,#e2e8f0)",padding:"10px 12px",width:280}}>
                  <div style={{display:"flex",flexWrap:"wrap",gap:3,maxHeight:180,overflowY:"auto"}}>
                    {EMOJIS_QUICK.map(e=>(
                      <button key={e} onClick={()=>{setText(p=>p+e);setShowEmoji(false);inputRef.current?.focus();}}
                        style={{width:36,height:36,borderRadius:8,border:"none",background:"transparent",fontSize:20,cursor:"pointer",transition:"transform 0.1s"}}
              ...
              <textarea ref={inputRef} value={text} onChange={handleTextChange}
                ...
                placeholder={editingMsg?"Edit message… (Esc to cancel)":replyTo?`Reply to ${activeInfo?.name||"message"}…`:`Message ${activeName}… (Enter to send)`}
              ...
              <button onClick={()=>editingMsg?submitEdit():sendMsg()} disabled={(!text.trim()&&!uploading)||sending} className="send-btn"
                style={{height:36,padding:"0 16px",borderRadius:12,background:text.trim()?(editingMsg?"#f59e0b":"#3b82f6"):"var(--input-bg)",color:text.trim()?"#fff":"var(--text-muted)",border:text.trim()?"none":"1px solid var(--border-color)",fontWeight:700,cursor:"pointer",transition:"all 0.15s",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
                {sending?<div style={{width:14,height:14,border:"2px solid rgba(255,255,255,0.5)",borderTopColor:"#fff",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>:editingMsg?<Check size={16}/>:<><Send size={15}/>{!isMobile&&<span style={{fontSize:13}}>Send</span>}</>}
              </button>
            </div>
            )}
          </div>
          ...
          <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",background:"var(--chat-bg)",flexDirection:"column",gap:16}}>
            ...
            <button onClick={()=>setShowNew(true)} style={{padding:"12px 28px",borderRadius:14,background:"linear-gradient(135deg,#3b82f6,#06b6d4)",color:"#fff",border:"none",fontWeight:700,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",gap:8,boxShadow:"0 4px 16px rgba(59,130,246,0.35)"}}>
